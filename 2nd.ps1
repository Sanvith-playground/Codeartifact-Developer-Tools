...
function Perform-SsoLogin {
    ...
    if ($session_valid) {
        ...
        Write-LogInfo "Initiating AWS SSO login via device code..."
        
        Write-Host "`n=== Action Required ===" -ForegroundColor Yellow
        Write-Host "1. Open the URL shown below in your browser."
        Write-Host "   (Tip: Use an Incognito/Private window if you need to switch users or force a password/MFA prompt)" -ForegroundColor Cyan
        Write-Host "2. Enter the device code provided."
        ...
        while ($attempt -le $max_retries) {
            aws sso login --profile "$PROFILE_NAME" --use-device-code 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) {
...
function Test-PackageInstall {
    ...
    if ($LASTEXITCODE -eq 0) {
         Write-LogSuccess "✔ Repository connection verified (found ${test_pkg})."
    } else {
